//
//  MyCoreDataTableViewController.swift
//  MyCoreData
//
...
//1) Add , UITableViewDelegate to MyCoreDataTableViewController

class MyCoreDataTableViewController: UITableViewController, UITableViewDelegate {
    
    //2) Add variable to hold NSManagedObject
    var mycoredataArray = [NSManagedObject]()
    ...
    func loaddb()
    {
        ...
        let fetchRequest = NSFetchRequest(entityName:"MyCoreData")
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let person = mycoredataArray[indexPath.row]
        cell.textLabel?.text = person.valueForKey("artist") as String?
    ...
    override func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if editingStyle == .Delete {
            ...
            context.deleteObject(mycoredataArray[indexPath.row])
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "UpDateMusic" {
            if let destination = segue.destinationViewController as?
                ViewController {
                    if let SelectIndex = tableView.indexPathForSelectedRow()?.row {
                        
                        let selectedDevice:NSManagedObject = mycoredataArray[SelectIndex] as NSManagedObject
                        destination.mycoredatadb = selectedDevice
                    }
            }
        }
